...
!   call ezcdf_error(status,varnam,'cdfgv','nf_inq_varid')
    ...
    if (ndims .eq. 3 .and. rank .ne. 3) then
      print "('% cdfgv: --E-- The variable ',a,' is 3 dimensional')",varnam
      return
    else if (ndims .eq. 2 .and. rank .ne. 2) then
      print "('% cdfgv: --E-- The variable ',a,' is 2 dimensional')",varnam
      return
    endif
    if (ndims .eq. 0 .and. sizes(1) .ne. 0) then
      print "('% cdfgv: --E-- The variable ',a,' is a Scalar')",varnam
      return
    else if (ndims .eq. 1 .and. rank .ne. 1 ) then
      print "('% cdfgv: --E-- The variable ',a,' is 1 dimensional')",varnam
      return
    endif
    ...
    case ('i')
      if(vartyp .ne. nf_int) then
        print "('% cdfgv: --E-- ',a,' is of type Integer !')", varnam
        return
      end if
    case ('l')
      if(vartyp .ne. nf_byte) then
        print "('% cdfgv: --E-- ',a,' is of type logical !')", varnam
        return
      end if
    case ('d')
      if(vartyp .ne. nf_double) then
        print "('% cdfgv: --E-- ',a,' is of type REAL*8 !')", varnam
        return
      end if
    case ('r')
      if(vartyp .ne. nf_real) then
        print "('% cdfgv: --E-- ',a,' is of type default REAL !')", varnam
        return
      end if
    case ('c')
      if(vartyp .ne. nf_char) then
        print "('% cdfgv: --E-- ',a,' is of type Character !')", varnam
        return
      end if
    end select
    status = 0
